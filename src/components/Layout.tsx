...
export default function Layout({ children }: LayoutProps) {
    ...
    useEffect(() => {
        ...
        const titles = {
            ...
            "/settings": translations[language].settingsTitle,
        };
    ...
    useEffect(() => {
        const protectedPages = ['/chart', '/key', '/settings'];
    ...
    const isLegalPage = ["/privacy", "/terms", "/about", "/documentation", "/information", "/settings"].includes(pathname);
    ...
    return (
        <div className="min-h-screen w-full bg-gradient-to-br from-slate-50 to-gray-100 dark:from-slate-900 dark:to-gray-800 flex flex-col">
            <Header />
            <div className={`flex-1 flex justify-center min-w-0 ${isHomeOrKey
                ? "items-center pt-14 sm:pt-16 lg:pt-20 pb-6 sm:pb-8 lg:pb-10"
                : isLegalPage
                    ? "items-start overflow-y-auto pt-16 sm:pt-20 lg:pt-24"
                    : "items-center pt-16 sm:pt-20 lg:pt-24"
                }`}>
                {children}
            </div>
            <Footer />
